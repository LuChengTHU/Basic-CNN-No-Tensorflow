...
def train_net(model, loss, config, inputs, labels, batch_size, disp_freq, loss_plot_list, acc_plot_list, iter_plot_list):
    ...
    for input, label in data_iterator(inputs, labels, batch_size):
        ...
        if iter_counter % disp_freq == 0:
            ...
            loss_plot_list.append(np.mean(loss_list))
            acc_plot_list.append(np.mean(acc_list))
            iter_plot_list.append(iter_counter)
            loss_list = []
            acc_list = []
            LOG_INFO(msg)


def test_net(model, loss, inputs, labels, batch_size, acc_test_plot_list):
    loss_list = []
    acc_list = []
    ...
    LOG_INFO(msg)
    acc_test_plot_list.append(np.mean(acc_list))
